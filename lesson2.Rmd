...
```{r heatmap}
###TASK 3: PATHWAY GO:0007159: leukocyte cell-cell adhesion
...
dds_tis <- DESeqDataSetFromMatrix(countData = counts,
                              colData = conditions,
                              design = ~ tissue)
dds_tis <- dds_tis[rowSums(counts(dds_tis)) > 20, ]
dds_tis <- DESeq(dds_tis)
res_tis <- results(dds_tis)
res_tis <- res_tis[order(res_tis[, 4]), ]
...
#setA <- rownames(subset(res, res$padj < 0.01))
#setB <- rownames(subset(res2, res2$padj < 0.01))
setA <- rownames(subset(res_tis, res_tis$padj < 0.01))
setB <- rownames(subset(res_cells, res_cells$padj < 0.01))
